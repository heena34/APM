...
class BinaryClassification:
    ...
    def executeAIOnTest(self,w1,w0):
        ...
        try:
            ...
            test_df = pd.read_csv('./server/Dataset/PM_test.txt', sep=" ", header=None)
            ...
            # Utilize ground truth dataset to generate labels for the test data.
            ...
            # Taking the last sequence for each id in the test data
            
            seq_array_test_last = [test_df[test_df['id']==id][self.sequence_cols].values[-BinaryClassification.sequence_length:] 
                                   for id in test_df['id'].unique() if len(test_df[test_df['id']==id]) >= BinaryClassification.sequence_length]
            ...
            # Similarly, Picking up the labels
            
            #print("y_mask")
            
            #it is used to take only the labels of the sequences that are at least 50 long
            y_mask = [len(test_df[test_df['id']==id]) >= BinaryClassification.sequence_length for id in test_df['id'].unique()]
            ...
            test_set.to_csv('./server/Output/binary_submit_test.csv', index = None)
            ...
            #plt.show()
            fig_verify.savefig("./server/Output/model_verify.png")
            ...
            response_data["actual_rul"] =  test_df.groupby('id')['RUL'].nth(-1)[y_mask].values.tolist()
            response_data["actual_rul_label1"] =  test_df.groupby('id')['label1'].nth(-1)[y_mask].values.tolist()
            response_data["pred_rul_label1"] = new_list
